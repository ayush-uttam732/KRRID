...
const ProfilePage = () => {
  ...
  const removeFriend = async (friendUsername: string) => {
    const { error } = await supabase
      ...
      .delete()
      .or(
        `and(user_username.eq.${user?.user_metadata?.username},friend_username.eq.${friendUsername}),and(user_username.eq.${friendUsername},friend_username.eq.${user?.user_metadata?.username})`
      );
    if (!error) {
      window.location.reload();
    }
  ...
  async function isUsernameAvailable(
    ...
    return !data.users.some(
      (user) =>
        ...
        user.user_metadata?.username !== currentUsername
    );
  }
      ...
      <div className="relative w-full h-64 bg-gradient-to-r from-blue-900 via-black to-blue-700 flex items-end px-16 py-8">
        ...
        <div className="relative z-10 flex items-end space-x-8 w-full">
          <Image
            ...
            width={128}
            height={128}
            className="rounded-lg border-4 border-sky-400 object-cover shadow-lg bg-gray-800"
            priority
          />
          <div className="flex-1">
            <div className="flex items-center space-x-3">
              ...
              <span className="inline-block bg-sky-600 text-white text-xs px-2 py-1 rounded">
                IN
              </span>
              <button
                onClick={() => {
      ...
      <div className="w-full flex justify-center bg-gray-900 border-b border-gray-800">
        <nav className="flex space-x-12 py-4 text-xl">
          {TABS.map((tab) => (
            <button
              key={tab.key}
              className={`font-semibold px-6 py-2 rounded-t transition-colors duration-200 ${
                activeTab === tab.key
            ...
            <div className="col-span-9 flex space-x-8">
              <div className="flex-1 bg-white/10 rounded-lg p-8 flex flex-col items-center shadow">
                ...
                <span className="text-4xl font-bold text-white">
                  {userStats?.rapid_rating ?? 400}
                </span>
                <span className="text-green-400 mt-1 text-lg">
                  +{(userStats?.games_won ?? 0) * 10}
                </span>
              </div>
              <div className="flex-1 bg-white/10 rounded-lg p-8 flex flex-col items-center shadow">
              ...
              <div className="flex-1 bg-white/10 rounded-lg p-8 flex flex-col items-center shadow">
                ...
                <span className="text-4xl font-bold text-white">
                  {userStats?.puzzle_rating ?? 400}
                </span>
                <span className="text-green-400 mt-1 text-lg">
                  +{(userStats?.puzzles_solved ?? 0) * 10}
                </span>
              </div>
            </div>
            <div className="col-span-3 flex flex-col space-y-8">
              <div className="bg-white/10 rounded-lg p-8 shadow flex flex-col items-center">
                ...
                <span className="text-gray-400">
                  Wins: {userStats?.games_won ?? 0}
                </span>
                <span className="text-gray-400">
                  Losses: {userStats?.games_lost ?? 0}
                </span>
              </div>
              <div className="bg-white/10 rounded-lg p-8 shadow">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button className="text-sky-400 hover:underline text-sm">
                    See All
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {friendsLoading ? (
                    ...
                    friends.slice(0, 8).map((friend, i) => (
                      <div
                        key={i}
                        className="w-12 h-12 rounded-full bg-gray-700 border-2 border-sky-400 flex items-center justify-center text-white text-lg font-bold"
                      >
                        F
                      </div>
                    ))
                      ...
                      <td className="py-2">
                        <div className="flex flex-col">
                          <span className="text-white">
                            {game.player_white || "Unknown"} vs{" "}
                            {game.player_black || "Unknown"}
                          </span>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400">
                        {userStats?.puzzles_solved ?? 0}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Puzzles Failed:</span>
                      <span className="text-red-400">
                        {userStats?.puzzles_failed ?? 0}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <div className="flex flex-col">
                        <span className="text-white font-semibold">
                          {result.user_metadata?.name ||
                            result.user_metadata?.username ||
                            "Unknown User"}
                        </span>
                        <span className="text-gray-400 text-sm">
                          @{result.user_metadata?.username} •{" "}
                          {getCountryName(
                            result.user_metadata?.country_code || "+1"
                          )}{" "}
                          • {result.user_metadata?.phone || "No phone"}
                        </span>
                      </div>
                      <button
                        onClick={() =>
                          addFriend(result.user_metadata?.username || "")
                        }
            ...
            <div className="flex gap-10">
              {[1, 2, 3].map((award) => (
                <div key={award} className="flex flex-col items-center">
                  <div className="w-20 h-20 bg-yellow-400 rounded-full flex items-center justify-center text-3xl font-bold text-white mb-2">
                    🏅
                  </div>
